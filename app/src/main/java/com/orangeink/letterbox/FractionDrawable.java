...
public class FractionDrawable extends Drawable {
    ...
    private List<CodeColors> codeColors;

    public FractionDrawable(List<CodeColors> codeColors) {
        mPaint = new Paint();
        this.codeColors = codeColors;
        setFraction(codeColors.get(0));
    }

    public void setFraction(CodeColors color) {
    ...
    public void draw(Canvas canvas) {
        ...
        for (int i = 0; i < codeColors.size(); i++) {
            ...
            if (i == 0) {
                ...
                canvas.drawPath(RoundedRect(codeColors.get(i).getStart(), codeColors.get(i).getEnd(), b.height(), true, false, true, false), mPaint);
            } else {
                codeColors.get(i).setStart(codeColors.get(i - 1).getEnd());
                codeColors.get(i).setEnd(codeColors.get(i).getStart() + x);
                canvas.drawPath(RoundedRect(codeColors.get(i).getStart(), codeColors.get(i).getEnd(), b.height(), false, false, false, false), mPaint);
            }
        }
    }
